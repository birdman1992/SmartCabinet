...
bool JSONObject::fromString(QString jsonText)
{
    ...
    QStdWString ws = jsonText.toStdWString();
    const wchar_t* ptr = ws.c_str();
    ptr=skip(ptr);
    switch(*ptr)
    {
        ...
        while(*ptr!=L'\"' && *ptr)
        {
            if ( *ptr == L'\\' )
            {
                ...
                case L'u' :
                    {
                        ptr++;
                        QString s = QString::fromStdWString(QStdWString(ptr, 4));
...
const wchar_t* JSONObject::parseValue(QString Key, const wchar_t* ptr)
{
    ...
    if ( *ptr )
    {
        ...
        if ( QStdWString(ptr, 4).compare(L"true")==0 )
        {
            add(newKey(Key), true);
            ptr+=4;
        }else
        if ( QStdWString(ptr, 5).compare(L"false")==0 )
        {
            add(newKey(Key), false);
            ptr+=5;
        }else
        if ( QStdWString(ptr, 4).compare(L"null")==0 )
        {
            add(newKey(Key));
            ptr+=4;
        }

...
//    qDebug()<<"prefix"<<prefix<<packData.toHex();
    ...
    int dataLen = len;
    if(prefix != 0x5aa5)
        dataLen = len<16?16:len;//data的长度
//    qDebug()<<packData.size()<<(dataLen+10);
    ...
    if(cks != packCks)
    {
        qDebug()<<"[ResponsePack] pack check error:"<<cks<<packCks<<(dataLen + 8);
